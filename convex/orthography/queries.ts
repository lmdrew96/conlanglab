...
export const historyList = query({
  ...
  handler: async (ctx, { languageId }) => {
    ...
    const entries = await ctx.db
      .query("stageHistory")
      .withIndex("by_language_stage", (q) => q.eq("languageId", languageId).eq("stage", "orthography"))
      .order("desc")
...
export const historyEntry = query({
  ...
  handler: async (ctx, { languageId, historyEntryId }) => {
    await requireLanguageOwner(ctx, languageId);
    return await reconstructAt<OrthographyStageData>(ctx, { languageId, stage: "orthography", historyEntryId });
  },
});
