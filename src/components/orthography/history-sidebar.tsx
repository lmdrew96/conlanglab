...
export function HistorySidebar({ languageId, stageLocked }: { languageId: Id<"languages">; stageLocked: boolean }) {
  ...
  const history = useQuery(api.orthography.queries.historyList, isAuthenticated ? { languageId } : "skip");
  const revert = useMutation(api.orthography.mutations.revertToHistoryEntry);
