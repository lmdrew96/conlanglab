...
export function HistorySidebar({ languageId, stageLocked }: { languageId: Id<"languages">; stageLocked: boolean }) {
  ...
  const history = useQuery(api.morphology.queries.historyList, isAuthenticated ? { languageId } : "skip");
  const revert = useMutation(api.morphology.mutations.revertToHistoryEntry);
