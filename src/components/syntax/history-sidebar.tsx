...
export function HistorySidebar({ languageId, stageLocked }: { languageId: Id<"languages">; stageLocked: boolean }) {
  ...
  const history = useQuery(api.syntax.queries.historyList, isAuthenticated ? { languageId } : "skip");
  const revert = useMutation(api.syntax.mutations.revertToHistoryEntry);
